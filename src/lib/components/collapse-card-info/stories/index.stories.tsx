...
export const WithPrefixAndSuffix: Story = {
  args: {
    title: 'Card with Prefix and Suffix',
    data: [
      {
        info: {
          description: 'Info with Prefix',
          Prefix: <span>🔍</span>,
        },
        value: {
          description: 'Value with Suffix',
          Suffix: <span>✓</span>,
        },
      },
      {
        info: {
          description: 'Info with both',
          Prefix: <span>📊</span>,
          Suffix: <span>!</span>,
        },
        value: {
          description: 'Value with both',
          Prefix: <span>$</span>,
          Suffix: <span>%</span>,
        },
      },
  ...
  play: async ({ args, canvasElement, step }) => {
    ...
    await step('Checking prefix and suffix are rendered', async () => {
      const firstInfo = canvas.getByText(args.data[0].info.description);
      const firstValue = canvas.getByText(args.data[0].value.description);

      expect(
        firstInfo.closest('div')?.textContent,
        'It is expected that the info prefix is rendered'
      ).toContain('🔍');
      expect(
        firstValue.closest('div')?.textContent,
        'It is expected that the value suffix is rendered'
      ).toContain('✓');

      const secondInfo = canvas.getByText(args.data[1].info.description);
      const secondValue = canvas.getByText(args.data[1].value.description);

      expect(
        secondInfo.closest('div')?.textContent,
        'It is expected that the info prefix and suffix are rendered'
      ).toContain('📊');
      expect(
        secondInfo.closest('div')?.textContent,
        'It is expected that the info suffix is rendered'
      ).toContain('!');
      expect(
        secondValue.closest('div')?.textContent,
        'It is expected that the value prefix is rendered'
      ).toContain('$');
      expect(
        secondValue.closest('div')?.textContent,
        'It is expected that the value suffix is rendered'
